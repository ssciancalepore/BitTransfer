		...
		if (ieee154e_isSynch() == TRUE){
			...
			if(idmanager_getIsDAGroot()){
				...
				if(ieee154e_vars.local_bit_counter_tx<=MSG_LENGTH){
					...
					if(ieee154e_vars.message_Tx[ieee154e_vars.local_bit_counter_tx] == 1){
						// transmit something
						...
						if (eb != NULL){
							...
							if(ieee154e_vars.local_bit_counter_tx==MSG_LENGTH-1){
								openserial_printInfo(COMPONENT_IEEE802154E,ERR_OK,
													 (errorparameter_t)ieee154e_vars.asn.bytes0and1,
							...
							ieee154e_vars.local_bit_counter_tx = ieee154e_vars.local_bit_counter_tx +1;
    ...
    */
    do { // this "loop" is only executed once
		...
		if (ieee154e_vars.asn.bytes0and1 > ANTIJAM_STARTING_TIME
			...
			ieee154e_vars.message_Rx[ieee154e_vars.local_bit_counter_rx] = 1;
			ieee154e_vars.local_bit_counter_rx = ieee154e_vars.local_bit_counter_rx + 1;
...
/*
 * Activated by a valid RX on a jammer, immediately transmit a packet to generate a collision
 */
//port_INLINE void activity_jam1(PORT_TIMER_WIDTH capturedTime) {
//
//    OpenQueueEntry_t* jam_pkt;
//	// change state
//    changeState(S_RXDATA_JAM);
//
//    //abort reception of the packet
//    // turn off the radio
//    radio_rfOff();
//
//    // record the captured time
...
//
//    //create new packet
//    jam_pkt = openqueue_getFreePacketBuffer(COMPONENT_SIXTOP);
//	if (jam_pkt != NULL){
//
//		openserial_printError(COMPONENT_IEEE802154E,ERR_WDACKDURATION_OVERFLOWS,
//		                         (errorparameter_t)capturedTime,
//		                         (errorparameter_t)ieee154e_vars.asn.bytes0and1);
//
//		jam_pkt->l2_frameType                     = IEEE154_TYPE_BEACON;
//		jam_pkt->l2_nextORpreviousHop.type        = ADDR_16B;
//		jam_pkt->l2_nextORpreviousHop.addr_16b[0] = 0xff;
//		jam_pkt->l2_nextORpreviousHop.addr_16b[1] = 0xff;
//		jam_pkt->l2_retriesLeft = 1;
//		jam_pkt->l2_numTxAttempts = 0;
//		jam_pkt->l1_txPower = 13;
//		jam_pkt->l2_IEListPresent = FALSE;
//		jam_pkt->l2_dsn = 0;
//		jam_pkt->SoC = 1;
//		// add a IEEE802.15.4 header
//		ieee802154_prependHeader(
//			jam_pkt,
//			jam_pkt->l2_frameType,
//			FALSE,
//			jam_pkt->l2_dsn,
//			&(jam_pkt->l2_nextORpreviousHop)
//		);
//
//		//eb->owner = COMPONENT_SIXTOP_TO_IEEE802154E;
//		ieee154e_vars.dataToSend = jam_pkt;
//		// configure the radio for that frequency
//		radio_setFrequency(ieee154e_vars.freq);
//		radio_setTxPower(jam_pkt->l1_txPower);
//
//		// load the packet in the radio's Tx buffer
